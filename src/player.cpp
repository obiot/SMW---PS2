...
void CPlayer::findspawnpoint(){
	for(int tries = 0; tries < 32; tries++){
		x = (float)(rand()%640);
		y = (float)(rand()%(480-TILESIZE*2));	// don't spawn too low
...
void CPlayer::draw(){
	...
	if(x < 0.0f)	//if the player is in the -both-sides-of-the-screen- area
		sprites[spr]->draw(640-(PW-(int)(x+PW)%640) - 1,   int(y) + y_rect.h - 4);
...
void CPlayer::collision_detection_map(){
	...
	if(x < 0.0f-PW)				//out of -on-both-sides- area
		x = 640+x;					//x is negativ!
	else if(int(x)+PW > 639)		//entered -on-both-sides- area (mind: the screen ends at 639 !!!)
		x = 0 - (639-int(x));
	...
	if(velx > 0){		//moving right	
		...
		if(x < 0.0f)
			tx = (640+int(x))/TILESIZE;	//test against tiles on the right side of the sceen. x is negativ!
		else
			tx = int(x)/TILESIZE;
	...
	if( x == -30.0f){					//special case: player hit a tile on the left side while being over the edge of the screen on the right side	
		x = 610.0f;	
		txl = 19;
	}
	else if( x == 640.0f){				//special case if we hit a tile on the other side of the screen (tile right side, player left side)
		txl = 0;
		x = 0;
	}
	else if(x < 0.0f){
		txl = (640+int(x))/TILESIZE;	//x is negativ (i keep forgetting that and write 640-int(x))
	}
	else{
		txl = int(x)/TILESIZE;
	}
