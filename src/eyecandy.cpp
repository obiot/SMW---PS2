...
void EC_Cloud::draw(){
	...
	if((x+w) > 640)
		spr->draw(0-(w-(int)(x+w)%640),   (int)y  + y_rect.h);
	else if(x < 0)
		spr->draw(640-(w-(int)(x+w)%640),   (int)y + y_rect.h);
}

void EC_Cloud::update(){
	x += velx;
	if((x+w) > 640)
		x = (float) ( 0-(w-(int)(x+w)%640) );
	else if( x < 0){
		x = (float) ( 640-(w-(int)(x+w)%640) );
	}
}
...
EC_GravText::EC_GravText(gfxFont *nfont, int nx, int ny, const char *ntext, float nvely){
	font = nfont;
	x = (float)(nx - (font->getWidth(ntext) >> 1)); // / 2
	y = (float)ny;
